...
class NeedleReach(PsmEnv):
    ...
    def _env_setup(self):
        ...
        # needle
        yaw = self._np_random.uniform(-0.5, 0.5) * np.pi
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'needle/needle_40mm.urdf'),
                            (workspace_limits[0].mean() + self._np_random.uniform(-0.5, 0.5) * 0.1,
                             workspace_limits[1].mean() + self._np_random.uniform(-0.5, 0.5) * 0.1,
